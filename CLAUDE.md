# CLAUDE.md — BDDM repo guide for Claude Code sessions

BDDM is a paper-to-Lean formalization pipeline: it ingests arxiv papers, extracts theorems, translates them to Lean 4, runs proof search via MCTS+Mistral, and produces a verification ledger of FULLY_PROVEN / INTERMEDIARY_PROVEN / AXIOM_BACKED / UNRESOLVED / FLAWED rows per paper.
...
2. **`scripts/paper_theory_builder.py`** — write `Desol/PaperTheory/Paper_<id>.lean` with paper-local definitions, axioms, and (since the cluster-A round) auto-emitted typeclass instances + aesop attributes on axioms.
...
4. **`scripts/prove_arxiv_batch.py`** — proof loop. Tries `_run_deterministic_file_micro_prover` first (line ~2407 catalog includes `aesop / simp_all / omega / linarith / norm_cast / gcongr / field_simp / polyrith / interval_cases`), then `_run_deterministic_micro_prover`, then state-MCTS with REPL.
5. **`scripts/build_claim_equivalence_review_queue.py`** → **`scripts/adjudicate_claim_equivalence.py`** (Mistral) → **`scripts/apply_claim_equivalence_adjudications.py`** — flip `claim_equivalence_verdict` on UNRESOLVED rows.
...
- 685+ tests in `tests/`. Run with `pytest tests/ -x -q`. Full suite ~5 min.
